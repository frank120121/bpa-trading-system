...
class MerchantAccount:
    ...
    async def handle_message_by_type(self, ws, KEY, SECRET, msg_json, order_no, conn, msg_type):
        ...
        if msg_type == 'system':
            await self._handle_system_type(ws, KEY, SECRET, msg_json, order_no, conn, order_details)
        else:
            await self._handle_other_types(ws, KEY, SECRET, msg_json, order_no, conn, msg_type, order_details)

    async def _handle_system_type(self, ws, KEY, SECRET, msg_json, order_no, conn, order_details):
    ...
    async def _handle_other_types(self, ws, KEY, SECRET, msg_json, order_no, conn, msg_type, order_details):
